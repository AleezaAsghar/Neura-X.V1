...
def is_valid_supabase_db_url(value):
    if not value:
        return False
    # Ignore template placeholders like <project-ref> / <password>.
    if "<" in value or ">" in value:
        return False
    return value.startswith("postgresql://") or value.startswith("postgres://")


DATABASE_BACKEND = "postgres" if is_valid_supabase_db_url(SUPABASE_DB_URL) else "sqlite"

_sqlite_connect = sqlite3.connect

def _convert_sqlite_query_for_postgres(query):
    """Convert SQLite-flavored SQL to Postgres-friendly SQL."""
    ...
    converted = re.sub(r"INTEGER\s+PRIMARY\s+KEY", "BIGSERIAL PRIMARY KEY", converted, flags=re.IGNORECASE)
    converted = converted.replace("datetime('now')", "NOW()")
    converted = re.sub(r'DEFAULT\s+"([^"]+)"', r"DEFAULT '\1'", converted)
    return converted


class DBCompatCursor:
    """Cursor wrapper that keeps legacy SQLite query behavior usable on Postgres."""

    def __init__(self, connection, cursor, is_postgres=False):
        self.connection = connection
        self._cursor = cursor
        self._is_postgres = is_postgres
        self._mock_results = None
        self.lastrowid = None
    ...
    def execute(self, query, params=None):
        ...
        # Ignore SQLite connection tuning PRAGMA statements.
        if normalized.startswith("pragma busy_timeout"):
            return self

        # Emulate PRAGMA table_info(table_name)
        if normalized.startswith("pragma table_info("):
            table_name = query[query.find("(") + 1:query.rfind(")")].strip().strip("'\"")
            info_cursor = self.connection._raw.cursor()
            info_cursor.execute(
                """
            ...
            cols = info_cursor.fetchall()
            self._mock_results = [
                (idx, col[0], col[1], 0 if col[2] == "YES" else 1, col[3], 0)
                for idx, col in enumerate(cols)
            ]
            info_cursor.close()
            return self

        # Emulate simple sqlite_master table-existence checks used in migrations.
        if "from sqlite_master" in normalized and "type='table'" in normalized:
            match = re.search(r"name\s*=\s*'([^']+)'", query, flags=re.IGNORECASE)
            table_name = match.group(1) if match else None
            exists_cursor = self.connection._raw.cursor()
            ...
            row = exists_cursor.fetchone()
            self._mock_results = [row] if row else []
            exists_cursor.close()
            return self

        converted_query = _convert_sqlite_query_for_postgres(query)
        self._cursor.execute(converted_query, params)

        # Preserve sqlite-like lastrowid behavior.
        if normalized.startswith("insert") and "returning" not in normalized:
            lastval_cursor = self.connection._raw.cursor()
            try:
                lastval_cursor.execute("SELECT LASTVAL()")
                row = lastval_cursor.fetchone()
                self.lastrowid = row[0] if row else None
            except Exception:
                self.lastrowid = None
            finally:
                lastval_cursor.close()

        return self

    def fetchone(self):
        if self._mock_results is not None:
...
class DBCompatConnection:
    """Connection wrapper to offer sqlite-like API surface across backends."""
    ...
    def cursor(self):
        if self._is_postgres:
            return DBCompatCursor(self, self._raw.cursor(cursor_factory=DictCursor), is_postgres=True)
...
def _database_connect(*args, **kwargs):
    if DATABASE_BACKEND == "postgres":
        if psycopg2 is None:
            raise RuntimeError("psycopg2-binary is required for SUPABASE_DB_URL backend")
        if not SUPABASE_DB_URL:
            raise RuntimeError("SUPABASE_DB_URL is required for Postgres backend")
        raw = psycopg2.connect(SUPABASE_DB_URL)
        return DBCompatConnection(raw, is_postgres=True)

    sqlite_path = args[0] if args else 'db.sqlite3'
    raw = _sqlite_connect(sqlite_path, **kwargs)
    return DBCompatConnection(raw, is_postgres=False)


# Route all sqlite3.connect(...) calls through compatibility layer.
...
def init_db():
    ...
    if DATABASE_BACKEND == "postgres":
        schema_path = Path(__file__).parent / "sql" / "schema_supabase.sql"
        if not schema_path.exists():
            raise FileNotFoundError(f"Missing schema file for Postgres bootstrap: {schema_path}")
...
def ensure_default_users(conn, cursor):
...
def migrate_database():
    ...
    if DATABASE_BACKEND == "postgres":
        try:
            ensure_default_users(conn, cursor)
            print("Postgres schema bootstrap complete")
        except Exception as e:
            print(f"Postgres migration error: {str(e)}")
            conn.rollback()
        finally:
            conn.close()
        return
...
When a patient mentions a symptom or concern:
...
- "Are you noticing any other symptoms along with this?"
...
- Typical consultation requires several exchanges.
- Each reply should contain exactly ONE question.
- Consider:
  - onset
  - duration
  - severity
...
- Incorporate relevant patient history when appropriate.
...
You MUST use this EXACT bullet-point structure — no extra text outside bullets.
Keep total response under 120 words.

Use exactly this format:

Based on what you've described: [one very short summary sentence]

Possible causes (preliminary, not a diagnosis):
• [one cautious possibility]
• [optional second possibility — only if very relevant]

Home care suggestions:
• [short suggestion 1]
• [short suggestion 2 — max 2]
...
• [warning sign 1]
• [warning sign 2]
• [warning sign 3 — max 3]

Follow-up:
• [one short monitoring sentence]

---

MODE 2: Instructor Mode (general medical education)

If the user asks a general medical knowledge question (e.g., "What is diabetes?"):

- Provide a clear educational explanation.
- Use short paragraphs or bullet points.
- No need for multi-turn questioning.
